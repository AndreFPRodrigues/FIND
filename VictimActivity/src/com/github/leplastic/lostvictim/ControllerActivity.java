...
public class ControllerActivity extends Activity { 
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnStartStop.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) { 
				boolean disabled = lostStatus == null
						|| lostStatus.equals("Disabled");
				if (disabled) {
	 ...
	 * 
	 * @param uri
	 *            Update uri to update a specific field or null to get all
	 *            values
	 */
	private void updateServiceStats(Uri uri) {
		Cursor c = getContentResolver()
				.query(uri == null ? Uri.parse("content://net.diogomarques.wifioppish.MessagesProvider/status")
						: uri, null, "", null, "");
		...
		if (c.moveToFirst()) {
			do {
				...
				} else if (c.getString(c.getColumnIndex("statuskey")).equals(
						"service")) {
					...
					runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							Drawable icon = getResources().getDrawable(
									isDisabled ? R.drawable.lightoff
											: R.drawable.lighton);
							imgLostStatus.setImageDrawable(icon);
							String text = getResources()
									.getString(
											isDisabled ? R.string.btnStartStop_do_enable
													: R.string.btnStartStop_do_disable);
